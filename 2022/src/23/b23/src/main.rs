...
fn main() {
    ...
    debug_println!("{board}");
    for round in 1.. {
        if let Some(next_board) = board.next_generation(round - 1) {
            board = next_board;
            debug_println!("== End of Round {round} ==");
            debug_println!("{board}");
        } else {
            println!("First round where nobody moves: {round}");
            break;
        }
    }
    println!(
